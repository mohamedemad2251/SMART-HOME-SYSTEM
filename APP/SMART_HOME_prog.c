...
STD_Type APP_SMART_HOME_u8App(void)									//Main application function to be run in main.
{
	...
	while(1)
	{
		if(LOC_u8Initialized == FALSE)			//If it's the first time
		{
			...
			LOC_u8ReturnValue = HAL_DC_MOTOR_u8Init(DC_MOTOR0);		//Initialize DC Motor (Please avoid using DC_MOTOR1, as it is used for the servo)
			...
			LOC_u8ReturnValue = MCAL_UART_u8CallbackFun(&callback_send_uart_fun);
